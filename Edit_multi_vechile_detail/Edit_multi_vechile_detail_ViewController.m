//
//  Edit_multi_vechile_detail_ViewController.m
//  PigeonShip
//
...
#import "Edit_multi_vechile_detail_ViewController.h"
#import "Bank_info_ViewController.h"
#import "Show_vechile_detail_ViewController.h"
@interface Edit_multi_vechile_detail_ViewController ()

@end

@implementation Edit_multi_vechile_detail_ViewController
...
- (void)Update_data:(NSNotification *)notification
{
    
    [[dict valueForKey:@"Vehicle Detail"] replaceObjectAtIndex:[[[NSUserDefaults standardUserDefaults]valueForKey:@"indexvalue"]integerValue] withObject:[[NSUserDefaults standardUserDefaults]valueForKey:@"detail_value_datya"]];
...
- (void)Delete_data:(NSNotification *)notification
{
    ...
    NSLog(@"index :---> %ld",(long)[[[NSUserDefaults standardUserDefaults]valueForKey:@"indexvalue"]integerValue]);
    
    NSLog(@"dicvalue  :--->%@",dict);
    
    
    
    [[dict valueForKey:@"Vehicle Detail"] removeObjectAtIndex:[[[NSUserDefaults standardUserDefaults]valueForKey:@"indexvalue"]integerValue]];
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter]
     addObserver:self
     selector:@selector(Update_data:)
     name:@"Update_dict_value"
     object:nil];
    
    
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(Delete_data:)
                                                 name:@"kuch_or_g"
                                               object:nil];
    ...
    mtableview.allowsMultipleSelectionDuringEditing = NO;
    ...
    Next_BTN.frame = CGRectMake((facke_view.frame.size.width-120)/2, 0,120, 40.0);
...
-(void)CalltheServer
{
    ...
    if ([API connectedToInternet]==YES) {
        ...
        if ([[dict valueForKey:@"status"] isEqualToString:@"true"]) {
            ...
            facke_view.frame= CGRectMake(30, mtableview.frame.origin.y+mtableview.frame.size.height+10,self.view.frame.size.width-60, 60.0);
...
-(void)ADD_MORE:(UIButton *)SENDER
{
    ...
    else
    {
        ...
        if ([[dict valueForKey:@"Vehicle Detail"]count]==0) {
            
            
            
            [dict setValue:arrr forKey:@"Vehicle Detail"];
        ...
        else
        {
            ...
            [[dict valueForKey:@"Vehicle Detail"]addObject:[arrr lastObject]];
        }
...
-(void)Next:(UIButton *)sender
{
    ...
    if ([[dict valueForKey:@"Vehicle Detail"]count]==0) {
        ...
        if ([[Car_Make_TXT.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]] isEqualToString:@""] || [[Car_Modal_TXT.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]] isEqualToString:@""] || [[Year_TXT.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]] isEqualToString:@""]|| [[Color_TXT.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]] isEqualToString:@""] ||[[Type_TXT.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]] isEqualToString:@""] ) {
            ...
            [all_data setValue:[dict valueForKey:@"Vehicle Detail"] forKey:@"vehicles"];
            
            NSData *jsonData = [NSJSONSerialization dataWithJSONObject:all_data options:0 error:nil];
            NSLog(@"%@",[[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding]);
            ALL_DATA=[[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding];
        ...
        else
        {
            ...
            NSLog(@"%@",[arrr lastObject]);
            
            NSLog(@"%@",[dict valueForKey:@"Vehicle Detail"]);
            ...
            [dict setValue:SSARRAY forKey:@"Vehicle Detail"];
...
-(void)Update_CallTheServer_data
{
    if ([API connectedToInternet]==YES) {
        ...
        NSString *responseString = [API makeCallPostData_ALL:[NSString stringWithFormat:@"security_token=%@&access_token=%@&user_id=%@&vehicle_data=%@",[Utility Pigeon_security_token],[[NSUserDefaults standardUserDefaults]valueForKey:@"access_token"],[[NSUserDefaults standardUserDefaults]valueForKey:@"User_ID"],ALL_DATA]                                                            :[NSString stringWithFormat:@"%@edit_vehicledetail",[Utility API_link_url]]];
        ...
        if ([[dict2 valueForKey:@"status"] isEqualToString:@"true"]) {
            ...
            alert = [[UIAlertView alloc] initWithTitle:@"Alert" message:[dict2 valueForKey:@"message"] delegate:self cancelButtonTitle:nil otherButtonTitles:@"OK", nil];
            [alert show];
        ...
        else
        {
            ...
            alert = [[UIAlertView alloc] initWithTitle:@"An error has occurred" message:[dict2 valueForKey:@"message"] delegate:nil cancelButtonTitle:nil otherButtonTitles:@"OK", nil];
            [alert show];
            }
...
{
    return [[dict valueForKey:@"Vehicle Detail"]count];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [[NSUserDefaults standardUserDefaults]setValue:[[dict valueForKey:@"Vehicle Detail"]objectAtIndex:indexPath.row] forKey:@"Data"];
    [[NSUserDefaults standardUserDefaults]synchronize];
    ...
    Show_vechile_detail_ViewController *objShow_vechile_detail_ViewController=[[Show_vechile_detail_ViewController alloc]init];
    
    [self.navigationController pushViewController:objShow_vechile_detail_ViewController animated:YES];
...
-(void)Back:(UIButton *)sender
{
     ...
     [[NSNotificationCenter defaultCenter] removeObserver:self name:@"Update_dict_value" object:nil];
    
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"kuch_or_g" object:nil];
