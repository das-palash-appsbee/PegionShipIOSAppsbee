//
//  Accepted_items_sender_detail.m
//  Pigeonship
//
...
@interface Accepted_items_sender_detail ()
{
...
@implementation Accepted_items_sender_detail
...
- (void)viewDidLoad {
    ...
    Sender_Mobile_number_LB_ST.text=@"Pigeon Contact";
    Sender_Mobile_number_LB_ST.backgroundColor=[UIColor clearColor];
...
//    [scrollview addSubview:right_Width];
    ...
    //
    //
    Item_type = [[UILabel alloc] init];
    Item_type.textColor = [UIColor blackColor];
    [Item_type setFrame:CGRectMake(20,Type.frame.origin.y+Type.frame.size.height,200, 22)];
    Item_type.backgroundColor=[UIColor clearColor];
    Item_type.textColor=[UIColor colorWithRed:180.0f/255.0f green:180.0f/255.0f blue:179.0f/255.0f alpha:1.0];
    Item_type.userInteractionEnabled=NO;
    [Item_type setFont:[UIFont fontWithName:@"bariol-regular" size:13.0]];
    
    
    NSString *string_Widths=@"Item type :";
    
    
    
    Item_type.text = string_Widths;
    
    
    Item_type.textAlignment=NSTextAlignmentLeft;
    [scrollview addSubview:Item_type];
...
//    
    ...
    [Package_price setFrame:CGRectMake(scrollview.frame.size.width-320,Item_type.frame.origin.y+Item_type.frame.size.height+15,300, 22)];
...
//
    ...
    Tracking_BTN = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    [Tracking_BTN addTarget:self
                   action:@selector(Tracking_action:)
         forControlEvents:UIControlEventTouchUpInside];
    Tracking_BTN.backgroundColor=[UIColor colorWithRed:252.0f/255.0f green:207.0f/255.0f blue:0.0f/255.0f alpha:1.0];
    [Tracking_BTN setTitle:@"Track Your Item" forState:UIControlStateNormal];
    Tracking_BTN.tag=1;
    [Tracking_BTN setFont:[UIFont fontWithName:@"bariol-regular" size:16.0]];
    [Tracking_BTN setTitleColor:[UIColor colorWithRed:107.0f/255.0f green:107.0f/255.0f blue:107.0f/255.0f alpha:1.0] forState:UIControlStateNormal];
    
    
    Tracking_BTN.layer.cornerRadius=10;
    Tracking_BTN.clipsToBounds = YES;
    Tracking_BTN.frame = CGRectMake((self.view.frame.size.width-150)/2, Package_price.frame.origin.y+30,150, 35);
    [scrollview addSubview:Tracking_BTN];
...
-(void)Tracking_action:(UIButton *)sender
{
    Pigeon_Posting_Map_view_ViewController *obj_Pigeon_Posting_Map_view_ViewController=[[Pigeon_Posting_Map_view_ViewController alloc]init];
    
    obj_Pigeon_Posting_Map_view_ViewController.Fetch_data=[dict valueForKey:@"Package_Detail"];
    obj_Pigeon_Posting_Map_view_ViewController.Pack_Key=Pack_Key;
    
    [self.navigationController pushViewController:obj_Pigeon_Posting_Map_view_ViewController animated:YES];
...
-(void)CalltheServer
{
    ...
    if ([API connectedToInternet]==YES) {
        ...
        if ( [[[dict valueForKey:@"Package_Detail"]valueForKey:@"pick_ready"]isEqualToString:@"yes"])
        {
            ...
            [Pick_ready setFrame:CGRectMake(scrollview.frame.size.width-220,Item_type.frame.origin.y+Item_type.frame.size.height,200, 22)];
        ...
        else
        {
            ...
            Pick_ready = [[UILabel alloc] init];
            Pick_ready.textColor = [UIColor blackColor];
            [Pick_ready setFrame:CGRectMake(20,Item_type.frame.origin.y+Item_type.frame.size.height,200, 22)];
            Pick_ready.backgroundColor=[UIColor clearColor];
            Pick_ready.textColor=[UIColor colorWithRed:180.0f/255.0f green:180.0f/255.0f blue:179.0f/255.0f alpha:1.0];
            Pick_ready.userInteractionEnabled=NO;
            [Pick_ready setFont:[UIFont fontWithName:@"bariol-regular" size:13.0]];
            ...
            [pic_date setFrame:CGRectMake(scrollview.frame.size.width-220,Item_type.frame.origin.y+Item_type.frame.size.height,200, 22)];
     ...
     ///////////////////////////// Max_pic_date Text lable\\\\\\\\\\\\\\\\\\\\\
            ...
            Sender_Mobile_number_LB_DYT.text=[[dict valueForKey:@"Package_Detail"]valueForKey:@"pigeon_mobile"];
            ...
            NSString *string_Widths=[NSString stringWithFormat:@"Item type :  %@",[[dict valueForKey:@"Package_Detail"]valueForKey:@"Item_type"]];
            
            NSMutableAttributedString *str_Widths = [[NSMutableAttributedString alloc] initWithString:string_Widths];
            
            [str_Widths addAttribute:NSForegroundColorAttributeName value:[UIColor colorWithRed:180.0f/255.0f green:180.0f/255.0f blue:179.0f/255.0f alpha:1.0] range:NSMakeRange(0, 15)];
            [str_Widths addAttribute:NSForegroundColorAttributeName value:[UIColor whiteColor] range:NSMakeRange(13, [string_Widths length]-14)];
            
            Item_type.attributedText = str_Widths;
            ...
            if ([[[dict valueForKey:@"Package_Detail"]valueForKey:@"classified"]isEqualToString:@"TRUE"])
            {
                ...
                if ([[[dict valueForKey:@"Package_Detail"]valueForKey:@"return"]isEqualToString:@"FALSE"]) {
                    ...
                    [Shipping_Price setFrame:CGRectMake(scrollview.frame.size.width-320,right_Width.frame.origin.y+right_Width.frame.size.height+15,300, 22)];
                    ...
                    Cancel_Radio_Btn_classified.frame = CGRectMake(5, right_Width.frame.origin.y+right_Width.frame.size.height+16,30, 30);
                    ...
                    [Cancel_order_LB_classified setFrame:CGRectMake(40,right_Width.frame.origin.y+right_Width.frame.size.height+20,self.view.frame.size.width, 20)];
                ...
                Shipping_Price = [[UILabel alloc] init];
                Shipping_Price.textColor = [UIColor blackColor];
                [Shipping_Price setFrame:CGRectMake(scrollview.frame.size.width-320,right_Width.frame.origin.y+right_Width.frame.size.height+10,300, 22)];
                Shipping_Price.backgroundColor=[UIColor clearColor];
                Shipping_Price.textColor=[UIColor whiteColor];
                Shipping_Price.userInteractionEnabled=NO;
                [Shipping_Price setFont:[UIFont fontWithName:@"bariol-regular" size:13.0]];
